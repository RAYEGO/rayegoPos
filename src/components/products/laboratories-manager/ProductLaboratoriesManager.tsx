...
function downloadTemplate() {
  ...
  link.href = url
  link.download = 'rayego-laboratorios-template.xlsx'
...
function parseSpreadsheet(buffer: ArrayBuffer) {
  ...
  return dataRows
    .map((cols, index): ParsedLaboratoryEntry => {
...
async function parseLaboratoryFile(file: File): Promise<ParsedLaboratoryEntry[]> {
...
function LaboratoryImportDialog({
  ...
  async function handleFile(file: File) {
    ...
    try {
      const parsed = await parseLaboratoryFile(file)
