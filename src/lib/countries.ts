...
const UNIQUE_COUNTRIES = Array.from(
  new Map(
    RAW_COUNTRIES.map((entry) => [entry.code, entry] as const),
  ).values(),
).sort((a, b) => {
  const an = stripDiacritics(a.name).toLowerCase()
  const bn = stripDiacritics(b.name).toLowerCase()
  if (an < bn) return -1
  if (an > bn) return 1
  return 0
})

export const COUNTRIES: CountryEntry[] = UNIQUE_COUNTRIES
