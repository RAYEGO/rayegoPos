...
import { estadoOrdenLabel, estadoOrdenBadgeVariant } from '@/types/rayegotech'
...
export function OrdenesServicioPage() {
  ...
  useEffect(() => {
    document.title = 'Servicio Técnico · RayegoTech'
  }, [])

  const [topTab, setTopTab] = useState<'ordenes' | 'equipos' | 'tecnicos' | 'garantias'>('ordenes')
  ...
  return (
    <TooltipProvider delayDuration={150}>
      <div className="space-y-4 p-4 max-h-[calc(100vh-40px)] flex flex-col min-h-0">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between flex-shrink-0">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              RayegoTech · Recepción, diagnóstico, presupuestos, reparación, pagos y entrega.
            </p>
          </div>
        ...
        <Tabs value={topTab} onValueChange={(v) => setTopTab(v as any)} className="flex-1 flex flex-col min-h-0">
          <TabsList className="w-max self-start flex-shrink-0">
            <TabsTrigger value="ordenes" className="h-11 px-5 text-sm">
              <ClipboardCheck className="mr-2 h-4 w-4" /> Órdenes de servicio
            </TabsTrigger>
            <Tooltip>
              <TooltipTrigger asChild>
                <span tabIndex={-1} className="inline-flex">
                  <TabsTrigger value="equipos" disabled className="h-11 px-5 text-sm">
                    <Wrench className="mr-2 h-4 w-4" /> Equipos
                  </TabsTrigger>
                </span>
              </TooltipTrigger>
              <TooltipContent side="bottom">Bloque posterior</TooltipContent>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger asChild>
                <span tabIndex={-1} className="inline-flex">
                  <TabsTrigger value="tecnicos" disabled className="h-11 px-5 text-sm">
                    <Zap className="mr-2 h-4 w-4" /> Técnicos
                  </TabsTrigger>
                </span>
              </TooltipTrigger>
              <TooltipContent side="bottom">Bloque posterior</TooltipContent>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <span tabIndex={-1} className="inline-flex">
                  <TabsTrigger value="garantias" disabled className="h-11 px-5 text-sm">
                    <ShieldCheck className="mr-2 h-4 w-4" /> Garantías
                  </TabsTrigger>
                </span>
              </TooltipTrigger>
              <TooltipContent side="bottom">Bloque posterior</TooltipContent>
          ...
          <TabsContent value="ordenes" className="mt-0 flex-1 min-h-0 overflow-hidden flex flex-col">
            <div className="flex-1 min-h-0 overflow-hidden">
              <AuthorizationGate permission="ordenesServicio.read">
                <Card className="flex-1 min-h-0 flex flex-col h-full">
                <CardHeader className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between flex-shrink-0">
                  <div>
                  ...
                  <div className="grid gap-4 xl:grid-cols-[1.4fr_0.8fr]">
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="LISTO_PARA_COBRO">Listo para cobro</SelectItem>
                        ...
                        <TableBody>
                          {visibleOrdenes.map((os) => (
                            ...
                            >
                              ...
                              <TableCell>
                                <Badge variant={estadoOrdenBadgeVariant(os.estado)}>
                                  {estadoOrdenLabel(os.estado)}
                                </Badge>
                              </TableCell>
                ...
                <div className="flex items-start justify-between gap-3 border-b bg-popover px-6 py-4">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <Badge variant={estadoOrdenBadgeVariant(selectedOrden.estado)}>
                        {estadoOrdenLabel(selectedOrden.estado)}
                      </Badge>
                    </div>
                  ...
                  <div className="overflow-x-auto px-4 pt-3">
                    <TabsList className="w-max">
                      ...
                      <TabsTrigger value="diagnosticos">
                        <BadgeCheck className="mr-1 h-4 w-4" /> Diagnóstico
                      </TabsTrigger>
                      <TabsTrigger value="presupuestos">
                        <Zap className="mr-1 h-4 w-4" /> Presupuesto
                      </TabsTrigger>
                            ...
                            {(selectedOrden.historialEstados as EstadoHistorialOrden[])
                              ...
                              .map((h) => (
                                <li key={h.id} className="ms-2">
                                  ...
                                  <div className="flex flex-wrap items-center gap-2">
                                    <Badge variant={estadoOrdenBadgeVariant(h.estadoNuevo)}>
                                      {estadoOrdenLabel(h.estadoNuevo)}
                                    </Badge>
                                    <span className="text-xs text-muted-foreground">
